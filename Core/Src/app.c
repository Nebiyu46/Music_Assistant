...
#define MAX_CATALOG_ITEMS SD_MAX_SONGS

static AppState_t app_state = APP_STATE_HOME;

static const Song_t *active_song = NULL;
...
static void run_playing_state(void)
{
...
#if AUDIO_USE_YIN
    AudioDSP_GetYinPeaks(audio_yin_input_buffer,
                         AUDIO_FFT_LENGTH,
                         peaks_from_yin,
                         n_peaks);
#else
    ...
    Gameplay_ValidateBricks(active_song,
                            1,
...
#if AUDIO_DEBUG_USB
    {
        ...
        len = sprintf(msg, "YIN notes: %s, %s, %s | FFT notes: %s, %s, %s | paused=%u\r\n",
...
void App_Run(void)
{
    const char *catalog_titles[MAX_CATALOG_ITEMS];
    int catalog_count;
    ...
    case APP_STATE_MENU:
        ...
        if (AppUI_PollOkPressed()) {
            if (menu_selection == 0) {
                SD_ScanSongs();
                catalog_count = SD_GetSongCount();
                for (int i = 0; i < catalog_count; i++) {
                    catalog_titles[i] = SD_GetSongTitle(i);
                }
                song_list_selection = 0;
    ...
    case APP_STATE_SONG_LIST:
        ...
        if (AppUI_PollOkPressed()) {
            const char *fname = SD_GetSongFilename(song_list_selection);
            if (SongStorage_LoadFromSD(fname)) {
    ...
    case APP_STATE_UPLOADING:
        if (UsbUpload_NeedsParsing()) {
            ...
            sprintf(filename, "song%d.txt", SD_GetSongCount() + 1);

            if (SD_SaveSong(filename, UsbUpload_GetBuffer(), UsbUpload_GetLength())) {
                SongStorage_ParseFromBuffer();
                uint16_t notes_found = (uint16_t)SongStorage_GetParsedCount();
                ...
                AppUI_DrawUploadDone(notes_found);
            }
            else {
